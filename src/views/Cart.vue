    ...
    <div class="center-set flex-col">
        ...
        <div class="flex flex-col mt-12 ">
            <div v-for="(item, index) in cartItems" :key="index" class="flex flex-col">
                <div class="flex space-x-6 ">
                    <div @click="deleteItems(index)" class="flex cursor-pointer select-none"><img src="../assets/icons/delete_button.svg" alt=""></div>
                    ...
                    <div class="flex space-x-28">
                        <div class="flex justify-between flex-col p-6">
                            <div class="flex flex-col">
                                ...
                                <div class="w-44 text-gray-400">{{ item.size }}</div>
                            </div>
                            <div class="flex flex-col ">
                        ...
                        <div class="flex space-x-4 mb-3">
                            ...
                            <div v-else @click="item.size = 'XS'"
                              class="border w-16 h-10 text-center text-gray-300 border-gray-300 border-2 p-1  select-none">XS
                            </div>
                            ...
                            <div v-else @click="item.size = 'S'"
                              class="border w-16 h-10 text-center text-gray-300 border-gray-300 border-2 p-1  select-none">S
                            </div>
                            ...
                            <div v-else @click="item.size = 'M'"
                              class="border w-16 h-10 text-center text-gray-300 border-gray-300 border-2 p-1  select-none">M
                            </div>
                        ...
                        <div class="flex space-x-4">
                            ...
                            <div v-else @click="item.size = 'L'"
                              class="border w-16 h-10 text-center text-gray-300 border-gray-300 border-2 p-1  select-none">L
                            </div>
                            ...
                            <div v-else @click="item.size = 'XL'"
                              class="border w-16 h-10 text-center text-gray-300 border-gray-300 border-2 p-1  select-none">XL
                            </div>
                        </div>
                    </div>
                        </div>
                        <div class="flex justify-between flex-col ">
                            <div class="flex space-x-4 p-8">
                                <div @click="plusMinus(item, -1)" class="flex cursor-pointer"> <img src="../assets/icons/minus.svg"
                                        alt=""></div>
                                <div :id="'numeric'+index" class="text-3xl">{{ item.count }}</div>
                                <div @click="plusMinus(item, 1)" class="flex mt-1 cursor-pointer select-none"> <img src="../assets/icons/plus.svg" alt=""></div>
                            </div>
        ...
        <div class="flex w-3/5 justify-between">
            ...
            <div class="w-1/2 mt-4 flex space-x-3">
                <button @click="LinkTo('/')" class="border text-center border-gray-500 border-2  w-1/2 h-10 text-black">CONTINUE
                    SHOPPING</button>
    ...
    computed: {
        ...
        cartItems() {
            ...
            for (let i = 0; i < this.count.length; i++) {
                cart.push(...this.products.filter(e => {
                    if(e.id == this.count[i].id){
                        e.count = this.count[i].count
                        e.size = this.count[i].size
                        return e
                    }
                }))
            }
